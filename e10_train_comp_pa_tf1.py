#!/usr/bin/python3
# -*- coding: utf-8 -*-
__version__ = '0.0.1' # Time-stamp: <2020-02-04T14:57:23Z>
...
parser.add_argument("--actor-actual-lr", default=0.0, type=float)
parser.add_argument("--actor-pseudo-lr", default=0.01, type=float)
...
def comp_train (sess, cenv, pred, actor, current, purpose):
    action = actor.model.predict(np.concatenate([current, purpose], axis=1))
    ...
    apredicted = sess.run(actor.predicted, feed_dict)
    ppurpose = purpose - actual + apredicted
    feed_dict[actor.purpose] = ppurpose
    pactions = []
    predicteds = []
    pactions.append(action)
    for i in range(actor.pseudo_action_iteration):
        pseudo_action, ppredicted = sess.run(
            [actor.temp_action, actor.predicted], feed_dict
        )
        pactions.append(pseudo_action)
        predicteds.append(ppredicted)
        feed_dict[actor.action] = pseudo_action
    pactions.pop()
    pseudo_action = np.clip(pseudo_action, -1.0, 1.0)
    feed_dict[actor.action] = pseudo_action
    ppredicted = sess.run(actor.predicted, feed_dict)
...
def run (sess):
    ...
    for cur_epoch in range(ARGS.epochs):
        ...
        for cur_step in range(ARGS.steps):
            ...
            action, pseudo_action, actual, ppredicted, \
                pred_loss, actor_loss, \
                pred_comp_loss, actor_comp_loss \
                = comp_train(sess, cenv, pred, actor, current, purpose)
            tloss = np.mean(np.square(true_action - action))
            ploss = np.mean(np.square(true_action - pseudo_action))
            aploss = np.mean(np.square(actual - purpose))
            pploss = np.mean(np.square(purpose - ppredicted))
            tloss_sum += tloss
            ploss_sum += ploss
            aploss_sum += aploss
            pploss_sum += pploss
