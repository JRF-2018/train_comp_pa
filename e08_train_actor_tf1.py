#!/usr/bin/python3
# -*- coding: utf-8 -*-
__version__ = '0.0.1' # Time-stamp: <2020-02-03T11:01:19Z>
...
parser.add_argument("--value-optimizer", default="adam", choices=["sgd", "rmsprop", "adam"])
parser.add_argument("--value-learning-rate", default=0.01, type=float)
...
class Actor:
    def __init__ (self, predictor=None, learning_rate=0.001,
                  ...
                  optimizer='sgd', value_optimizer='adam',
        ...
        if value_optimizer == 'rmsprop':
            self.value_optimizer = tf.train.RMSPropOptimizer(value_learning_rate)
        elif value_optimizer == 'adam':
            self.value_optimizer = tf.train.AdamOptimizer(value_learning_rate)
        else:
            self.value_optimizer = tf.train.GradientDescentOptimizer(value_learning_rate)
        self.model = self._build_network()
    ...
    def _compile_graph (self, model):
        ...
        self.action = tf.placeholder(
            tf.float32, shape=(None, self.num_actions))
        self.action_v = tf.Variable(tf.zeros((ARGS.batch_size, self.num_actions)), dtype=tf.float32)

        self.action_assign = self.action_v.assign(self.action)
        pout = pred.model(tf.concat([self.current, self.action_v], axis=1))
        self.value_loss = K.mean(K.square(self.purpose - pout))
        self.value_minimize = self.value_optimizer.minimize(
            self.value_loss, var_list=[self.action_v]
        )
    ...
    def calc_pseudo_action(self, sess, current, purpose, action):
        ...
        sess.run(tf.variables_initializer(
            self.value_optimizer.variables() + [self.action_v])
        )
        sess.run(self.action_assign, {self.action: action})
        for i in range(self.pseudo_action_iteration):
            _ = sess.run(self.value_minimize, feed_dict)
        pseudo_action = sess.run(self.action_v)
...
def run (sess):
    ...
    actor = Actor(predictor=pred,
                  ...
                  value_optimizer=ARGS.value_optimizer,
