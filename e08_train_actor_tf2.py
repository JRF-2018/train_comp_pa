#!/usr/bin/python3
# -*- coding: utf-8 -*-
__version__ = '0.0.1' # Time-stamp: <2020-02-03T10:01:55Z>
...
class Actor:
    ...
    def calc_pseudo_action(self, current, purpose, action):
        ...
        if optimizer == 'rmsprop':
            ...
            opt = SGD(lr)
        action_v = tf.Variable(action, dtype=tf.float32)
        current_c = tf.constant(current)
        purpose_c = tf.constant(purpose)
        for i in range(self.pseudo_action_iteration):
            with tf.GradientTape() as tape:
                pout = self.predictor.model \
                    (tf.concat([current_c, action_v], axis=1))
                loss = K.mean(K.square(purpose_c - pout))
            grads = tape.gradient(loss, [action_v])
            opt.apply_gradients(zip(grads, [action_v]))

        return action_v.numpy()
