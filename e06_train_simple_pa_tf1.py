#!/usr/bin/python3
# -*- coding: utf-8 -*-
__version__ = '0.0.1' # Time-stamp: <2020-02-04T13:35:44Z>
...
parser.add_argument("--simplest", default=False, action="store_true")
...
def comp_train (sess, cenv, pred, actor, current, purpose,
    ...
    if simplest:
        ...
        for i in range(actor.pseudo_action_iteration):
            pseudo_action, predicted = sess.run(
...
def run (sess):
    ...
    # if ARGS.pre_train == 0 and ARGS.random_action and not ARGS.simplest:
    #     _ = actor.model.predict(np.zeros((ARGS.batch_size, 4)))
        ...
        for cur_step in range(ARGS.steps):
            ...
            action, pseudo_action, pactual, predicted, \
                ...
                = comp_train(sess, cenv, pred, actor, current, purpose,
                             ARGS.simplest, ARGS.random_action)
