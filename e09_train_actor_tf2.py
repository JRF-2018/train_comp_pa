#!/usr/bin/python3
# -*- coding: utf-8 -*-
__version__ = '0.0.1' # Time-stamp: <2020-02-04T15:35:19Z>
...
def run ():
    ...
    for cur_epoch in range(ARGS.epochs):
        ...
        for cur_step in range(ARGS.steps):
            ...
            if ARGS.train_true:
                action = actor.model(batch[0]).numpy()
                pseudo_action = action
                actual = batch[0][:, actor.num_states:]
                loss = actor.model.train_on_batch(batch[0], batch[1])
            else:
